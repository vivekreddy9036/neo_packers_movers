        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {testimonials.map((t, i) => (
            <motion.div
              ...
              className="bg-white border border-slate-200 rounded-xl p-7 flex flex-col gap-6 hover:shadow-soft hover:-translate-y-0.5 transition-all duration-300"
            >
              {/* Stars + Verified badge */}
              <div className="flex items-center justify-between">
                <div className="flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, s) => (
                    <Star key={s} size={18} weight="fill" className="text-amber-400" />
                  ))}
                </div>
              ...
              {/* Quote */}
              <p className="text-ink-900 text-[15px] leading-relaxed flex-1">
                &ldquo;{t.quote}&rdquo;
              </p>

              {/* Author */}
              <div className="flex items-center gap-3 pt-5 border-t border-slate-100">
                <div className="h-10 w-10 flex-shrink-0 grid place-items-center rounded-full bg-slate-100 text-slate-600 font-semibold text-sm">
                  {initials(t.name)}
                </div>
                <div>
